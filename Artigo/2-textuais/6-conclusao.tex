...
\begin{itemize}
    \item \textbf{Protótipo de Software:} Um protótipo funcional do hiperinstrumento Aerochord, implementado em C++/JUCE, que realize o \textit{pipeline} de captura de vídeo, detecção de pose via MediaPipe, análise de gestos, mapeamento gesto–som e geração de mensagens MIDI 2.0 (com \textit{fallback} para MIDI 1.0), conforme a arquitetura proposta no Capítulo~\ref{sec:arquitetura_proposta}.
...
Para estruturar o trabalho a ser realizado, apresenta-se a seguir um plano de atividades dividido em fases sequenciais, com estimativa de duração em semanas. Cada fase foca em um conjunto de tarefas articuladas para garantir progresso incremental e avaliação contínua.

\begin{enumerate}
    \item \textbf{Fase 1 – Desenvolvimento do Núcleo do Sistema (Semanas 1–5):}  
    Prevê-se iniciar pela implementação dos módulos básicos: captura de vídeo (abstração multiplataforma), integração do MediaPipe para extração de \textit{landmarks} de mãos, e módulo de geração de mensagens MIDI 2.0 em C++/JUCE, incluindo configuração de \textit{handshake} MIDI-CI e fallback para MIDI 1.0. Durante essa fase, serão validadas as interfaces internas e testados protótipos iniciais de envio de pacotes UMP.
    \item \textbf{Fase 2 – Implementação da Lógica de Interação (Semanas 4–7):}  
    Será desenvolvido o módulo de análise de gestos, baseado em máquina de estados finitos, para reconhecimento robusto de gestos de pinça, movimentos de seleção de oitava e controles contínuos. Em paralelo, implementa-se o módulo de mapeamento gesto–som, conforme a Tabela~\ref{tab:mapeamento_preliminar} e o mapeamento refinado descrito em Capítulo~\ref{sec:mapeamento_gesto}. Nesta etapa, serão aplicados mecanismos de minimização de inconsistências e filtros de suavização nos \textit{landmarks} para mitigar falsos positivos.
    \item \textbf{Fase 3 – Integração e Testes Iniciais (Semanas 8–10):}  
    Consiste em integrar todos os módulos implementados, estabelecer a comunicação por filas \textit{lock-free} para minimizar latência e verificar o fluxo completo do pipeline. Serão realizados testes funcionais para confirmar que captura, detecção, análise de gestos, mapeamento e geração MIDI operam em conjunto sem travamentos ou atrasos perceptíveis. Ajustes iniciais de parâmetros (\textit{buffers}, prioridades de \textit{threads}, \textit{thresholds} de confiança) serão feitos com base em medições de \textit{profiling}.
    \item \textbf{Fase 4 – Execução da Avaliação com Usuários (Semanas 11–12):}  
    Planeja-se recrutar participantes representativos do público-alvo (músicos sem treinamento formal e pessoas com mobilidade reduzida) e conduzir sessões de uso do protótipo em ambiente controlado. Será aplicado protocolo de avaliação conforme metodologia definida em Capítulo~\ref{sec:avaliacao_usuarios_contextos}, coletando dados objetivos (latência, precisão de gestos) e subjetivos (SUS, NASA-TLX, entrevistas). Os logs de eventos gestuais e de mensagens MIDI serão registrados para análise posterior.
