...
O requisito técnico mais crítico para qualquer Instrumento Musical Digital é a manutenção de uma latência imperceptível para o intérprete, garantindo a sensação de causalidade imediata entre o gesto físico e o evento sonoro. Conforme os estudos levantados sobre interfaces baseadas em visão computacional, a latência de ponta a ponta nesse paradigma varia tipicamente entre 30 ms e 60 ms \cite{lugaresi2019mediapipe, hassan2019gestureaudio}. Portanto, estabelecer e alcançar uma meta abaixo da barreira dos 30 milissegundos (ms) torna-se o marco ideal para superar as limitações técnicas comuns da área e preservar a tocabilidade em aplicações musicais interativas.

Para medir essa métrica, o Aerochord foi executado em um computador pessoal, equipado com processador AMD Ryzen 7 7800X3D, 32 GB de memória RAM DDR5, placa de vídeo NVIDIA GeForce RTX 4070 Ti e sistema operacional Ubuntu 24.04.4 LTS. A captura visual foi realizada por meio de uma \textit{webcam} Logitech C505, que possui uma resolução de 720p (1280x720 pixels) e taxa de captura de 30 quadros por segundo. O sistema operou utilizando o modo analítico da aplicação (\texttt{--eval}). Neste modo, o \textit{software} regista marcações temporais (\textit{timestamps}) exatas no momento em que um quadro de vídeo é capturado (entregue pelo subsistema de vídeo) e, em seguida, regista o momento exato em que a mensagem MIDI 2.0 correspondente é enviada para o sequenciador do sistema operacional. Cabe ressaltar que essa métrica isola a latência do \textit{pipeline} de \textit{software} — da captura do quadro ao envio da mensagem MIDI —, não incluindo a latência intrínseca da câmera (exposição do sensor e transferência de dados via barramento USB) nem a renderização de áudio do sintetizador receptor, ambas independentes da implementação.
...
A análise estatística do arquivo de registro gerado pelo sistema demonstrou resultados altamente satisfatórios, superando a meta estabelecida. Os dados recolhidos foram:
\begin{itemize}
...
O modo de transição fluida de notas (\textit{legato}), suportado pela sobreposição temporal de eventos através do algoritmo de programação, desempenhou a sua função de maneira satisfatória, assegurando transições limpas. Foram registradas falhas esporádicas no acoplamento das notas apenas em ocasiões onde os movimentos do utilizador excederam o limiar de tolerância algorítmico nas zonas de fronteira (zonas mortas).
...
Para garantir o rigor metodológico da avaliação qualitativa, a sessão com o participante foi guiada por um protocolo de tarefas dividido em três etapas principais:
\begin{enumerate}
    \item \textbf{Familiarização Livre (Aclimatização):} Inicialmente, o participante teve um período livre para explorar o sistema, com o objetivo de compreender a área de captura da câmera e o mapeamento básico entre os gestos das mãos e o retorno sonoro.
    \item \textbf{Tarefas Estruturadas (Controle e Precisão):} Em seguida, foi solicitado que o participante executasse ações intencionais e específicas. Isso incluiu o acionamento isolado e repetido de eventos sonoros (para testar a responsividade), a variação na velocidade dos movimentos (para avaliar a dinâmica e a sensibilidade do sistema) e a manutenção de posturas contínuas no espaço (para testar a manipulação de parâmetros expressivos, como modulação).
    \item \textbf{Improvisação Musical (Uso Ecológico):} Por fim, o participante foi instruído a utilizar o \textit{Aerochord} de forma contínua em um contexto de improvisação musical, visando avaliar a fadiga física, o conforto ergonômico e a expressividade geral do instrumento em um cenário de uso real.
\end{enumerate}

Foi a partir da execução deste roteiro que as percepções e os \textit{feedbacks} do participante foram coletados e analisados. O teste foi conduzido num ambiente não controlado (doméstico). Foi fornecida a P1 uma breve instrução verbal, com duração inferior a 5 minutos, explicando a separação funcional de papéis entre as mãos (a mão de articulação executa notas e a mão de controle controla a região das oitavas e o volume), atribuídos automaticamente pela ordem de entrada em quadro.
