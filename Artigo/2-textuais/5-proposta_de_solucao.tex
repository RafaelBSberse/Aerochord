...
O sistema prevê os seguintes casos de uso principais, que são acionados simultaneamente dependendo da lateralidade (mão dominante ou não dominante) do usuário:
\begin{itemize}
    ...
    \item \textbf{Controlar Volume e Oitava:} Ações de macrocontrole, operadas pela mão não dominante, que configuram a região tonal e a intensidade global do som.
...
\begin{table}[htbp]
    ...
    {\scriptsize
     ...
     \begin{tabularx}{\textwidth}{L{2.5cm} L{2cm} L{4.5cm} X}
        \toprule
        \textbf{Função Musical} & \textbf{Lateralidade} & \textbf{Gesto Utilizado} & \textbf{Justificativa} \\
        \midrule
        Seleção de Oitava 
            & Não Dominante 
...
Observa-se que a mão não dominante foi minimamente utilizada, o que abre margem para expansões futuras de controle (como acionamento de acordes inteiros ou mudança de programas). O mapeamento discrimina claramente ações discretas (ativação de nota) e modulações contínuas.
